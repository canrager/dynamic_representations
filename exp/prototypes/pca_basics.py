...
if __name__ == "__main__":
    ...
    act_LbD, act_LBPD, mask_BP, trunc_seq_length = compute_or_load_llm_artifacts(
        model_name, num_stories, do_omit_BOS_token, do_truncate_seq_length
    )
    ...
    story_centered_PD = (
        story_PD - means_LD[layer_idx][None, :]
    ...
    for p in range(0, story_PD.shape[0], step_size):
        ax.plot(
            range(num_components), frac_variance_reconstructed_PC[p], label=f"Token {p}"
        )
    ax.legend()
    ax.set_xlabel("Number of PCA components")
    ax.set_ylabel("Fraction of variance reconstructed")
    ax.set_title(
        f"Fraction of variance reconstructed for story {story_idx} in layer {layer_idx}\n{model_name}"
    )
    plt.savefig(
        os.path.join(
            ARTIFACTS_DIR,
            f"frac_variance_reconstructed_{model_str}_story_{story_idx}_layer_{layer_idx}.png",
        )
    )
    print(
        f"Saved figure to {os.path.join(ARTIFACTS_DIR, f'frac_variance_reconstructed_{model_str}_story_{story_idx}_layer_{layer_idx}.png')}"
    )
    plt.close()
    ...
    tokens_centered_BPD = (
        tokens_BPD - data_mean_D[None, None, :]
    )  # Broadcast across batch and position
    ...
    ax.set_title(
        f"Variance explained for fixed token positions across stories in layer {layer_idx}"
    )
    ...
    for b in range(num_stories):
        ax.plot(
            range(num_tokens),
            min_components_required_BP[b],
            label=f"Story {story_idxs[b]}",
        )
    ax.legend()
    ...
    ax.set_title(
        f"Number of PCA components required to reconstruct {reconstruction_threshold*100}% of variance for stories {story_idxs} in layer {layer_idx}"
    )
