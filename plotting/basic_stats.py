...
def plot_stats_across_saes(results):
            ...
            if metric_key == "fraction_alive":
                ...
                for sae_name, result in dataset_data.items():
                    ...
                    for act_type in activation_types:
                        act_type_name = act_type[:-3]  # Remove .pt extension
                        if (
                            act_type_name in shown_temporal_act_types["codes"]
                            and metric_key in result[act_type]
                        ):
    ...
    if codes_legend_entries:
        ...
        for combo_label in sorted(codes_legend_entries):
            ...
            linestyle = "-" if "temporal" in sae_name else "--"
            codes_handles.append(
                plt.Line2D(
                    [0], [0], color=combo_colors[combo_label], marker="o", linestyle=linestyle
                )
            )
        ...
        codes_legend = fig.legend(
            codes_handles,
            codes_labels,
            title="Codes Metrics",
            loc="center left",
            bbox_to_anchor=(1.02, codes_y_pos),
        )
    ...
    if recons_legend_entries:
        ...
        if codes_legend_entries:
            ...
            recons_y_pos = (
                legend_y_top - codes_legend_height - legend_y_spacing - recons_legend_height / 2
            )
        else:
            recons_y_pos = legend_y_top - recons_legend_height / 2

        recons_legend = fig.legend(
            recons_handles,
            recons_labels,
            title="Recons Metrics",
            loc="center left",
            bbox_to_anchor=(1.02, recons_y_pos),
        )
...
def export_stats_to_csv(results):
    """
    Export aggregated statistics to CSV format.

    For each SAE/dataset combination, computes mean and CI across all sequence positions
    for each metric and exports to CSV with columns:
    sae_name, data_name, metric_mean, metric_ci for each metric
    """
    import numpy as np
    import pandas as pd
    ...
    for result_key, result in results.items():
        ...
        for act_type in activation_types:
            act_type_name = act_type[:-3]  # Remove .pt extension
            act_data = result[act_type]

            row_data = {
                "sae_name": sae_name,
                "data_name": data_name,
                "activation_type": act_type_name,
            }

            # Process each metric
            for metric_key, metric_data in act_data.items():
                if metric_key == "fraction_alive":
                    # Special case: single value, no aggregation needed
                    row_data[f"{metric_key}_mean"] = metric_data
                    row_data[f"{metric_key}_ci"] = np.nan
                else:
                    # Sequence-dependent metrics
                    if isinstance(metric_data, dict) and "score" in metric_data:
                        scores = np.array(metric_data["score"])

                        # Compute mean across sequence positions
                        mean_val = np.mean(scores)
                        row_data[f"{metric_key}_mean"] = mean_val

                        # Compute CI across sequence positions if available
                        if "ci" in metric_data and metric_data["ci"] is not None:
                            ci_values = np.array(metric_data["ci"])
                            # Use mean of CIs as aggregate CI
                            mean_ci = np.mean(ci_values)
                            row_data[f"{metric_key}_ci"] = mean_ci
                        else:
                            row_data[f"{metric_key}_ci"] = np.nan
    ...
    base_cols = ["sae_name", "data_name", "activation_type"]
    metric_cols = [col for col in df.columns if col not in base_cols]
    metric_cols.sort()  # Sort metric columns alphabetically

    df = df[base_cols + metric_cols]

    # Save to CSV
    output_path = "artifacts/results/basic_stats_aggregated.csv"
    df.to_csv(output_path, index=False)
    print(f"Exported aggregated statistics to {output_path}")
