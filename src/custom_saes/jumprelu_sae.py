...
class JumpReluSAE(base_sae.BaseSAE):
    def __init__(
        ...
        self.threshold = nn.Parameter(th.zeros(d_sae, dtype=dtype, device=device))

    def encode(self, x: th.Tensor):
        pre_acts = x @ self.W_enc + self.b_enc
        mask = pre_acts > self.threshold
        acts = mask * th.nn.functional.relu(pre_acts)
        return acts
...
def load_gemma_scope_jumprelu_sae(
    ...
    pt_params = {k: th.from_numpy(v).cpu() for k, v in params.items()}
