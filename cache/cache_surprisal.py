...
def load_surprisal_sentences():
    """Load surprisal sentences from JSON file."""
    json_path = os.path.join("artifacts", "text_inputs", "surprisal.json")
...
def cache_surprisal_llm_activations(cfg):
    ...
    sentences = load_surprisal_sentences()
    ...
    with open(os.path.join(folder_dir, "sentences.pt"), "wb") as f:
...
def load_surprisal_llm_activations(cfg, return_target_dir=False):
    ...
    artifacts, target_dir = load_matching_activations(
        source_object=cfg,
        target_filenames=["sentences"],
        target_folder=cfg.env.activations_dir,
        recency_rank=0,
        compared_attributes=["llm", "data"],
    )
    if not return_target_dir:
        return artifacts["sentences"]
    else:
        return artifacts["sentences"], target_dir
